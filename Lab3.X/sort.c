...
ListItem *SelectionSort(ListItem* list)
{
    ...
    while (first->nextItem) {
        ListItem *s = first->nextItem;
        while (s) {
            if (strcmp(first->data, s->data) > 0) {
                LinkedListSwapData(first, s);
            }
            s = s->nextItem;
        }
...
ListItem *InsertionSort(ListItem* list)
{
    ...
    while (last->previousItem) {
        ListItem *LU = last->previousItem;
        if (strcmp(LU->data, last->data) < 0) {
            last = LU;
        }
        else {
            ListItem *s = last;
